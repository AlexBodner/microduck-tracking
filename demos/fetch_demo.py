...
from microduck import CWD, ROOT, Microduck
from owner import Owner
from policy import FetchPolicy
from view import PANEL_W, View
from vision import BallDetector, build_tracker, focal_px
...
tracker = build_tracker(TRACK_EVERY)
...
def score_grab(t):
    """How far the thrown ball was when the beak came down."""
    global grabbed_at
...
def score_touch(t):
    """Did the beak move the ball? Read from the ball, not judged by eye."""
    if grabbed_at is None:
        return
    moved = float(np.linalg.norm(duck.ball_position()[:2] - grabbed_at[:2]))
    print(
...
if TRIAL:
    policy.on_grab = score_grab
    policy.on_touch = score_touch

throws = 0
next_throw = FIRST_THROW
tracked = None
detections = None
dump_rows = []
    ...
    if step % TRACK_EVERY == 0:
        ...
        if os.environ.get("DUMP_DETECTIONS"):
            for box, confidence in zip(detections.xyxy, detections.confidence):
                dump_rows.append((step, *box, confidence))
...
if os.environ.get("DUMP_DETECTIONS"):
    ...
    np.savez_compressed(
        cache, rows=np.array(dump_rows, dtype=np.float64), n_frames=steps
    )
    print(f"wrote {cache}: {len(dump_rows)} detections over {steps} frames")
